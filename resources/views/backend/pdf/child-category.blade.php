    ...
    <table class="table table-bordered">
        ...
        <tbody>
            @php
                $i = 1;
            @endphp
            @foreach ($data as $child_category)
                <tr>
                    <td>{{ $i++ }}</td>
                    <td>{{ $child_category->child_category }}</td>
                    <td>{{ $child_category->slug }}</td>
                    <td>{{ $child_category->childcategory->categories ?? '-' }}</td>
                    <td>
                        @php
                            $img_path = !empty($child_category->img_path)
                                ? public_path('backend/upload/image/child-category/' . $child_category->img_path)
                                : public_path('backend/assets/images/default_image.jpg');
                        @endphp
                    ...
                    <td>
                        ...
                        @elseif ($child_category->status == 'inactive')
                    ...
                    <td>{{ $child_category->updated_at }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
