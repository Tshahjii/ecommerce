...
@section('content')
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center flex-wrap">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <a class="btn btn-warning btn-label right" href="{{ route('subcategory-bulkupload') }}">
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                <li>
                                    <a class="dropdown-item" href="{{ route('sub-category-excel') }}">
                                        <i class="ri-file-excel-2-line me-2"></i> Download Excel
                                    </a>
                                </li>
                                <li>
                                    <a class="dropdown-item" href="{{ route('sub-category-pdf') }}">
                        ...
                        <table id="buttons-datatables" class="display table table-bordered" style="width:100%">
                            ...
                            <tbody>
                                @php
                                    $i = 1;
                                @endphp
                                @foreach ($sub_category as $values)
                                    <tr>
                                        <td>{{ $i++ }}</td>
                                        <td>{{ $values->sub_category }}</td>
                                        <td>{{ $values->slug }}</td>
                                        <td>{{ $values->childCategory->child_category }}</td>
                                        <td>
                                            @php
                                                $img_path = !empty($values->img_path)
                                                    ? asset('backend/upload/image/sub-category/' . $values->img_path)
                                                    : asset('backend/assets/images/default_image.jpg');
                                            @endphp
                                            <div class="avatar-xs justify-content-between bg-light rounded p-1 me-2">
                                                <img src="{{ $img_path }}" alt="sub Category" class="img-fluid d-block">
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="hstack gap-3 flex-wrap">
                                                <a href="javascript:void(0);" class="link-success fs-15 update-sub-category"
                                                    data-id="{{ $values->id }}" data-bs-toggle="modal"
                                                    data-bs-target=".bs-example-modal-center"><i
                                                        class="ri-edit-2-line"></i></a>
                                                <a href="{{ route('delete-sub-category', ['id' => $values->id]) }}"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('create-sub-category') }}" method="POST" autocomplete="off"
                        ...
                        <div class="row">
                            <div class="col-md-6 col-12">
                                <div class="mb-3">
                                    ...
                                    <label for="customer-name" class="col-form-label">Sub Category:</label>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="category_id" id="category_id" class="form-select" data-choices
                                        ...
                                        @foreach ($child_category as $values)
                                            <option value="{{ $values->id }}">{{ $values->child_category }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
    ...
    <script>
        ...
        $(document).on('input', '#sub_category', function() {
        ...
        $(document).on('click', '.update-sub-category', function() {
            ...
            $.ajax({
                url: "{{ route('get-sub-category', ':id') }}".replace(':id', id),
                type: "GET",
                dataType: "json",
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        $('#category_id').val(response.data.child_category_id);
