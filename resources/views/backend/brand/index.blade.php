...
@section('content')
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center flex-wrap">
                    ...
                    <div class="d-flex gap-2">
                        <button class="btn btn-info btn-label right create-brands-btn" data-bs-toggle="modal"
                            data-bs-target=".bs-example-modal-center">
                            <i class="ri-edit-2-fill label-icon align-middle fs-16 ms-2"></i>
                            Create
                        </button>
                        <a class="btn btn-warning btn-label right" href="{{ route('brands-bulkupload') }}">
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                <li>
                                    <a class="dropdown-item" href="{{ route('brands-excel') }}">
                                        <i class="ri-file-excel-2-line me-2"></i> Download Excel
                                    </a>
                                </li>
                                <li>
                                    <a class="dropdown-item" href="{{ route('brands-pdf') }}">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @php
                                                $img_path = !empty($values->img_path)
                                                    ? asset('backend/upload/image/brands/' . $values->img_path)
                                        ...
                                        <td>
                                            <div class="hstack gap-3 flex-wrap">
                                                <a href="javascript:void(0);" class="link-success fs-15 update-brands"
                                                    data-id="{{ $values->id }}" data-bs-toggle="modal"
                                                    data-bs-target=".bs-example-modal-center"><i
                                                        class="ri-edit-2-line"></i></a>
                                                <a href="{{ route('delete-brands', ['id' => $values->id]) }}"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('create-brands') }}" method="POST" autocomplete="off"
                        enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3">
                            <input type="hidden" id="b_id" name="id">
                            <label for="customer-name" class="col-form-label">Brands Name:</label>
    ...
    <script>
        $(document).on('click', '.create-brands-btn', function() {
            $('#b_id').val('');
            $('#brands').val('');
            $('#slug').val('');
            $('.status').val('');
            $('#varyingcontentModalLabel').text('Create Brands');
        });
        $(document).on('input', '#brands', function() {
            let brands = $(this).val();
            let ucfirst = brands.toLowerCase().replace(/\b\w/g, function(char) {
        ...
        $(document).on('click', '.update-brands', function() {
            let id = $(this).data('id');

            $.ajax({
                url: "{{ route('get-brands', ':id') }}".replace(':id', id),
                type: "GET",
                dataType: "json",
                success: function(response) {
                    if (response.status === 'success') {
                        $('#b_id').val(response.data.id);
                        $('#brands').val(response.data.brands);
